...
type ModalMode = "add_asset" | "edit_asset" | "add_liability" | "edit_liability" | null;
...
//colors for the asset donut chart: one per category
...
// ---------------------------------------------------------------------------
// Custom tooltip for the history line chart
// ---------------------------------------------------------------------------
...
const NetWorth = () => {
    ...
    const fetchSummary = async () => {
        ...
        try {
            setLoading(true);
            const res = await axios.get<NetworthSummary>(`${API_BASE}/api/networth`, { headers: authHeaders() });
            setSummary(res.data);
            setError(null);
        } catch (err: any) {
    ...
    const fetchHistory = async () => {
        ...
        try {
            ...
            //chart stays empy on failure
        }
    };

    const recordSnapshot = async () => {
        try {
            await axios.post(`${API_BASE}/api/networth/snapshot`, null, { headers: authHeaders() });
        } catch {
    ...
    // refresh history after snapshot is recorded
    useEffect(() => {
        if (!loading) fetchHistory();
    }, [loading]);

    // ---------------------------------------------------------------------------
    // Pie chart data: asset category breakdown
    // ---------------------------------------------------------------------------

    const buildPieData = () => {
        if (!summary) return [];
        const data: { name: string; value: number; color: string; category: string }[] = [];

        if (summary.portfolio_value > 0) {
            data.push({ name: "Portfolio", value: summary.portfolio_value, color: CATEGORY_COLORS.portfolio, category: "portfolio" });
        }

        const categoryTotals: Record<string, number> = {};
        for (const asset of summary.assets) {
            categoryTotals[asset.category] = (categoryTotals[asset.category] || 0) + asset.balance;
        }
    ...
    const openAddAsset = () => {
        ...
        setEditingId(null);
        setFormName(""); setFormCategory("cash"); setFormBalance("");
        setFormError(null);
    };
    ...
    const openEditAsset = (a: NetworthAsset) => {
        ...
        setEditingId(a.id);
        setFormName(a.name); setFormCategory(a.category); setFormBalance(String(a.balance));
        setFormError(null);
    };
    ...
    const openAddLiability = () => {
        ...
        setEditingId(null);
        setFormName(""); setFormCategory("credit_card"); setFormBalance("");
        setFormError(null);
    };
    ...
    const openEditLiability = (l: NetworthLiability) => {
        ...
        setEditingId(l.id);
        setFormName(l.name); setFormCategory(l.category); setFormBalance(String(l.balance));
        setFormError(null);
    };
    ...
    const handleModalSubmit = async () => {
        ...
        if (isNaN(balance) || balance < 0) { setFormError("Enter a valid balance."); return; }
        ...
        try {
            ...
            if (modalMode === "add_asset") {
                await axios.post(`${API_BASE}/api/networth/assets`, body, { headers: authHeaders() });
            } else if (modalMode === "edit_asset" && editingId) {
                await axios.put(`${API_BASE}/api/networth/assets/${editingId}`, body, { headers: authHeaders() });
            } else if (modalMode === "add_liability") {
                await axios.post(`${API_BASE}/api/networth/liabilities`, body, { headers: authHeaders() });
            } else if (modalMode === "edit_liability" && editingId) {
                await axios.put(`${API_BASE}/api/networth/liabilities/${editingId}`, body, { headers: authHeaders() });
            }
            closeModal();
            await fetchSummary();
    ...
    const handleDeleteAsset = async (id: string) => {
        ...
        try {
            await axios.delete(`${API_BASE}/api/networth/assets/${id}`, { headers: authHeaders() });
            await fetchSummary();
            await recordSnapshot();
            await fetchHistory();
        } catch {
    ...
    const handleDeleteLiability = async (id: string) => {
        ...
        try {
            await axios.delete(`${API_BASE}/api/networth/liabilities/${id}`, { headers: authHeaders() });
            await fetchSummary();
            await recordSnapshot();
            await fetchHistory();
        } catch {
    ...
    const modalTitle =
        modalMode === "add_asset" ? "Add Asset"
        : modalMode === "edit_asset" ? "Edit Asset"
        : modalMode === "add_liability" ? "Add Liability"
        : "Edit Liability";
            ...
            <div className="home-card nw-card">
                <div className="home-content">
                    ...
                    <div className="nw-summary-grid">
                        <div className={`nw-stat-card nw-stat-primary ${netWorthPositive ? "positive" : "negative"}`}>
                            <div className="nw-stat-label">Net Worth</div>
                            <div className={`nw-stat-value nw-stat-large ${netWorthPositive ? "positive" : "negative"}`}>
                                {formatCurrency(s.net_worth)}
                            </div>
                            <div className="nw-stat-sub">
                                {netWorthPositive ? "Assets exceed liabilities" : "Liabilities exceed assets"}
                            </div>
                        </div>
                        <div className="nw-stat-card">
                            <div className="nw-stat-label">Portfolio</div>
                            <div className="nw-stat-value">{formatCurrency(s.portfolio_value)}</div>
                        </div>
                        <div className="nw-stat-card">
                            <div className="nw-stat-label">Total Assets</div>
                            <div className="nw-stat-value positive">{formatCurrency(s.total_assets)}</div>
                        </div>
                        <div className="nw-stat-card">
                            <div className="nw-stat-label">Total Liabilities</div>
                            <div className={`nw-stat-value ${s.total_liabilities > 0 ? "negative" : ""}`}>
                                {formatCurrency(s.total_liabilities)}
                            </div>
                        </div>
                    </div>

                    {/* Charts row: history line + asset donut */}
                        ...
                        <div className="nw-chart-card nw-history-chart">
                            ...
                            {history.length >= 2 ? (
                                ...
                                <div className="nw-chart-empty">
                                    Net worth history will appear here after multiple daily visits.
                                </div>
                            )}
                        </div>
                                    ...
                                    <div className="nw-pie-legend">
                                        {pieData.map((entry) => (
                                            <div key={entry.category} className="nw-pie-legend-row">
                                                <span className="nw-pie-dot" style={{ background: entry.color }} />
                                                ...
                                                <span className="nw-pie-amount">{formatCurrency(entry.value)}</span>
                                            </div>
                                        ))}
                                    </div>
                                </>
                            ) : (
                                <div className="nw-chart-empty">Add assets to see distribution.</div>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="nw-lists-row">
                        ...
                        <div className="nw-list-section">
                            ...
                            <div className="nw-entry-row nw-portfolio-row">
                                ...
                                <span className="nw-entry-balance positive">{formatCurrency(s.portfolio_value)}</span>
                                <div className="nw-entry-actions" />
                            </div>

                            {s.assets.length === 0 && (
                                <div className="nw-empty-hint">No manual assets yet. Click "+ Add Asset" to start.</div>
                            )}

                            {s.assets.map((a) => (
                                <div key={a.id} className="nw-entry-row">
                                    <span className="nw-category-badge">{CATEGORY_LABELS[a.category] || a.category}</span>
                                    <span className="nw-entry-name">{a.name}</span>
                                    <span className="nw-entry-balance">{formatCurrency(a.balance)}</span>
                                    <div className="nw-entry-actions">
                                        <button className="nw-edit-btn" onClick={() => openEditAsset(a)} title="Edit">
                                            Edit
                                        </button>
                                        <button className="nw-delete-btn" onClick={() => handleDeleteAsset(a.id)} title="Delete">
                                            ✕
                                        </button>
                                    </div>
                                </div>
                            ...
                            <div className="nw-total-row">
                                ...
                                <span className="nw-total-value positive">{formatCurrency(s.total_assets)}</span>
                            </div>
                        </div>
                        ...
                        <div className="nw-list-section">
                            <div className="nw-list-header">
                                ...
                                <button className="add-shares-btn" onClick={openAddLiability}>+ Add Liability</button>
                            </div>

                            {s.liabilities.length === 0 && (
                                <div className="nw-empty-hint">No liabilities tracked. Click "+ Add Liability" to add one.</div>
                            )}

                            {s.liabilities.map((l) => (
                                <div key={l.id} className="nw-entry-row">
                                    <span className="nw-category-badge nw-badge-liability">{CATEGORY_LABELS[l.category] || l.category}</span>
                                    <span className="nw-entry-name">{l.name}</span>
                                    <span className="nw-entry-balance negative">{formatCurrency(l.balance)}</span>
                                    <div className="nw-entry-actions">
                                        <button className="nw-edit-btn" onClick={() => openEditLiability(l)} title="Edit">
                                            Edit
                                        </button>
                                        <button className="nw-delete-btn" onClick={() => handleDeleteLiability(l.id)} title="Delete">
                                            ✕
                                        </button>
                                    </div>
                                </div>
                            ...
                            <div className="nw-total-row">
                                ...
                                <span className={`nw-total-value ${s.total_liabilities > 0 ? "negative" : ""}`}>
                                    {formatCurrency(s.total_liabilities)}
                                </span>
                            </div>
                        </div>
                    ...
                    <div className="modal-container nw-modal">
                        ...
                        <div className="modal-body">
                            ...
                            <input
                                ...
                                type="text"
                                placeholder={isAddingAsset ? "e.g. Chase Savings" : "e.g. Visa Credit Card"}
                                value={formName}
                                onChange={(e) => { setFormName(e.target.value); setFormError(null); }}
                            />
                            <label className="modal-labe" style={{ marginTop: "0.75rem" }}>Category</label>
                            <select
                            ...
                            <label className="modal-labe" style={{ marginTop: "0.75rem" }}>Balance ($)</label>
                            <input
                                className="modal-input"
                                type="number"
                                min="0"
                                ...
                                value={formBalance}
                                onChange={(e) => { setFormBalance(e.target.value); setFormError(null); }}
                                onKeyDown={(e) => e.key === "Enter" && handleModalSubmit()}
                            />
                            {formError && <p className="modal-error">{formError}</p>}
                        </div>
                        <div className="modal-footer">
                            <button className="modal-cancel-btn" onClick={closeModal} disabled={submitting}>
                                Cancel
                            </button>
                            <button
